...
if st.button("Add Block"):
    ...
    # and `amount` values
    new_block = Block(
        record=Record(sender=input_sender,receiver=input_receiver,amount=float(input_amount)),
        creator_id=42,
...
if st.button("Validate Chain"):
    validity = "VALID" if pychain.is_valid() else "INVALID"
    st.write(f"This chain is {validity}")
